...
float enc_count[COUNT_MOTORS] = {0.0, 0.0};      // счетчик колес
...
float enc_count_last[COUNT_MOTORS] = {0.0, 0.0}; // счетчик колес прошлый
...
SimpleKalmanFilter kf(0.1, 1, 0.1);
Servo grip;
...
void loop() {
  grip.write(180);
  setLCDInfo(0, 0, "GO");   // вывод на экран значения

  resetEncCount();          // сброс счетчиков энкодеров
  // едем по линии до тех пор пока не увидим сонаром предмет ближе 10 см
  while (getRangeSonar() > 0.10) {
    moveRobotLine();
    setLCDInfo(0, 0, "SNR: " + (String)getRangeSonar());   // выводим значения на экран
  }
  
  resetEncCount();          // сброс счетчиков энкодеров
  float dist = 0.08;
  while (impulse2meters(enc_count[LEFT]) <= dist && impulse2meters(enc_count[RIGHT]) <= dist)
  {
    Serial.println(impulse2meters(enc_count[LEFT]));
  ...
  // проверяем состояние датчика цвета
  if(!digitalRead(SENSOR_COLOR_PIN)){
    setLCDInfo(0, 1, "Color: White");      // вывод на экран значения
    grip.write(0);                         // закрываем клешню
  }
  else
  {
    setLCDInfo(0, 1, "Color: Black");      // вывод на экран значения
    grip.write(180);                       // открываем клешню
  }
...
void setLCDInfo(int colomn, int str, String s) {
  //int s_len = s.length() + 1;
  //char char_array[s_len];
  //s.toCharArray(char_array, s_len);
  lcd.setCursor(colomn, str);           //переводим курсор
  lcd.print(s);                         //выводим текст
}

float getRangeSonar() {
  float kf_value = kf.updateEstimate(sonar.read() / 100.0);
  Serial.print("Range: ");
  Serial.println(kf_value);
  return kf_value;
}
...
void moveRobotLine() {
  ...
  if (t > TIME_PUB_PERIOD_MS) {
    ...
    speed_actual[LEFT] = impulse2rad(getEncCountInt(LEFT)) / ((float)t / 1000.0);
    speed_actual[RIGHT] = impulse2rad(getEncCountInt(RIGHT)) / ((float)t / 1000.0);
    ...
    enc_count_last[LEFT] = enc_count[LEFT];
    enc_count_last[RIGHT] = enc_count[RIGHT];

    last_time_pub = millis(); // фиксируем время последней публикации
  }
}

float getEncCountInt(int side) {
  return enc_count[side] - enc_count_last[side];
}

void resetEncCount(){
  for(int i = 0; i < COUNT_MOTORS; i++)
  {
    enc_count[i] = 0.0;
    enc_count_last[i] = 0.0;
  }
}
